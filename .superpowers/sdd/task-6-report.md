...
All six call public `verify_prelock_qualification()` through temporary real
direct-child Git S/R history. Every replay emits canonical self-hashed audit-v2
bytes, no bootstrap/retry field, `launch_qualified=false`, and all authority
false. Governance retry remains false for every identity.
