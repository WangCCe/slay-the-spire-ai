...
def build_child_env(args):
    ...
    combat_action_relative_candidate_registration = getattr(
        args, "combat_action_relative_candidate_registration", None
    )
    if sum(
        bool(value)
        for value in (
            ...
            combat_action_relative_candidate_registration,
        )
    ) > 1:
        raise ValueError(
    ...
    if combat_latent_candidate_registration:
        env["STS_COMBAT_RL_LATENT_CANDIDATE_REGISTRATION"] = str(
            combat_latent_candidate_registration
        )
    else:
    ...
    if combat_action_relative_candidate_registration:
        env["STS_COMBAT_RL_ACTION_RELATIVE_CANDIDATE_REGISTRATION"] = str(
            combat_action_relative_candidate_registration
        )
    else:
        env.pop("STS_COMBAT_RL_ACTION_RELATIVE_CANDIDATE_REGISTRATION", None)
...
def parse_args():
    ...
    parser.add_argument(
        "--combat-latent-candidate-registration",
        default=None,
        help=(
            "Explicit registration passed as "
    ...
    parser.add_argument(
        "--combat-action-relative-candidate-registration",
        default=None,
        help=(
            "Explicit registration passed as "
