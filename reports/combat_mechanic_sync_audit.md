...
| Card-play-triggered relic effects | Fresh Letter Opener `Defend` rows and Bird-Faced Urn `Demon Form` / Havoc-top `Barricade` rows | synced for Letter Opener skill-triggered lethal support | synced for Letter Opener skill damage and Bird-Faced Urn direct/Havoc-top power healing | n/a unless fallback starts estimating non-attack skill damage or power healing directly | n/a unless timing starts projecting these relic triggers directly | no direct RL guard/reward surface found this round | Stone Calendar remains a separate end-turn relic candidate from the same fresh distribution. |
