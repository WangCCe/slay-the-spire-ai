...
| Effect-played draw-pile cards | Distilled Chaos clean row with visible top `Defend`, `Headbutt+`, and `Defend` | n/a unless lethal search starts estimating potion-played top-card lines | synced for Distilled Chaos visible top-card potion simulation without Havoc-forced exhaust semantics | n/a | n/a | no direct RL guard/reward surface found this round | Empty-draw reshuffle remains state-insufficient because the trace lacks discard-pile order; multi-monster targeted attacks remain random-target boundaries for monster fields only. |
