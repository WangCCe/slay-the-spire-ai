...
| Exhaust-triggered block/damage | Havoc, nested Havoc, Feel No Pain, Juggernaut divergence tests plus fresh Havoc top-energy, top-Clash playability, and Shockwave self-exhaust evidence | synced for deterministic top attacks, top-card exhaust damage, visible top energy skills, and top-Clash playability | synced for nested top-card Havoc recursion and top-Clash playability | synced for deterministic and nested Havoc top-card block, top-Clash playability, Feel No Pain fallback priority, and direct self-exhaust Feel No Pain block | no independent Havoc path found; timing reads synced estimators | synced for survival and shared block guards where guard can prove target/effect, including top-Clash playability | Havoc random-target and empty-draw discard-reshuffle boundaries remain conservative by design. |
| Effect-played draw-pile cards | Distilled Chaos clean row with visible top `Defend`, `Headbutt+`, and `Defend`; Havoc empty-draw discard reshuffle clean row; live Havoc top `Clash` blocked by remaining non-attack hand card | synced where Havoc top-card attacks participate in lethal search; potion-played top-card lethal remains n/a | synced for Distilled Chaos visible top-card potion simulation without Havoc-forced exhaust semantics and for special top-card play conditions such as Clash; Havoc empty-draw reshuffle is treated as random/opaque in the oracle | synced for Havoc visible-top fallback block scoring when a top attack is blocked by special play conditions | n/a | synced for Havoc visible-top attack guard and survival block when a top attack is blocked by special play conditions | Empty-draw reshuffle now records discard-pile context but remains non-deterministic because live shuffle order and target choice are not provable; multi-monster targeted attacks remain random-target boundaries for monster fields only. |
...
- 2026-06-11: Live clean-trace evidence showed `Havoc` consuming a visible
  top `Clash` while a non-attack card remained in hand. Live exhausted the top
  card without dealing damage or triggering attack-play effects, while the old
  oracle and live estimators treated the visible top attack as deterministic.
  `Clash` playability is now checked against the remaining hand after the
  effect source leaves hand. The diagnostic oracle, `FastCombatSimulator`,
  `CombatEndingDetector` affordable-damage and exact lethal paths, Ironclad
  fallback block scoring, and RL Havoc/survival guards all stop counting the
  blocked top attack's damage or Ornamental Fan block while preserving Havoc's
  top-card consumption and effect-exhaust triggers.
- 2026-06-10: A follow-up fresh 5-game clean batch produced eight post-cutoff
  `PlayCardAction` divergences. The highest-frequency class was five
  `player.block` rows where live `Ornamental Fan` counter state granted or did
  not grant 4 block while `sim_divergence.py` relied on its local
  `_attacks_played_this_turn` tracker. The live decision estimators already
  have focused Ornamental Fan counter coverage in fast simulation, fallback,
  timing, and RL survival guards, so this round synced the diagnostic oracle:
  Ornamental Fan projection now reconciles the live relic `counter` snapshot
  with local observed attack count before deciding whether the next attack
  triggers 4 block.
...
- 2026-06-10: Havoc empty-draw discard reshuffle is now classified as a
  ...
  discard pile; live shuffled discard into draw, played a shuffled `Strike`,
  and dealt 15 damage into one vulnerable Cultist. `sim_divergence.py` now
  records discard-pile summaries in snapshots for evidence, but treats this
  empty-draw Havoc path as random/opaque and ignores player/monster state diffs
  from the shuffled card. Live estimators remain conservative unless the
  draw-pile top card is visible and therefore provable.
...
- 2026-06-08: Stone Calendar end-turn AOE damage is now synced from
  ...
  counter `7`; after end turn, the counter advanced to `8` and Guardian HP
  dropped by 52. `sim_divergence.py` now applies the same 52 AOE damage and
  counter advance. `FastCombatSimulator` carries `stone_calendar_counter` in
  `SimulationState`, includes it in `state_key`, and applies the damage in
  `project_end_turn_effects`, allowing outcome scoring to see kills or reduced
  incoming threat caused by the relic. Lethal detection, scalar fallback, and
  RL guard surfaces were left unchanged because this mechanism is an end-turn
  projection effect rather than a card-sequence damage estimate or incoming
  survival estimate.
- 2026-06-08: Letter Opener and Bird-Faced Urn card-play relic triggers
  are now synced from fresh floor-16 Hexaghost divergence rows into the
  diagnostic oracle and live combat estimators. `sim_divergence.py` now tracks
  Letter Opener's skill counter, applies the third-skill 5 AOE damage through
  monster block, resets the counter, and heals 2 HP from Bird-Faced Urn when a
  power is played directly or by a known top-card effect such as Havoc.
  `FastCombatSimulator` carries both relic states and applies the same skill
  damage and power healing. `CombatEndingDetector` carries Letter Opener
  counter state so a skill can prove an exact lethal line when it triggers the
  AOE damage. Bird-Faced Urn was not added to lethal search because the heal
  does not directly establish a kill sequence.
- 2026-06-08: Distilled Chaos `play_top_cards` is now synced from the
  fresh floor-12 potion row into the diagnostic oracle and beam potion
  simulation. The live potion played visible top draw-pile cards `Defend`,
  `Headbutt+`, and `Defend`, gaining deterministic block and dealing the
  visible attack damage while the old oracle and beam potion path treated the
  potion as utility-only. `sim_divergence.py` now applies visible top draw-pile
  cards for `play_top_cards` potions, while keeping Havoc's forced-exhaust
  semantics behind a separate `exhaust_by_effect` flag. Multi-monster targeted
  attacks played by the potion are treated as random-target boundaries for
  monster fields only. `HeuristicCombatPlanner` passes `context` into potion
  simulation so `FastCombatSimulator` can read `context.game.draw_pile` for the
  same top-card execution path.
