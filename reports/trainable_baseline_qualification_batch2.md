...
- Monitoring: 50-second marker/debug/error polls. The causal invalid-command signature was detected on the next poll and the exact batch processes were stopped: wrapper PID `40464` and game-agent PID `34728`.
...
- The two production Python processes were confirmed absent after `Stop-Process -Id 34728,40464 -Force`. ModTheSpire remained running; no batch/game Python remained.

## Run Outcomes
- Stop count: 15 new AI markers (14,731 to 14,746), 15 unique paired Ironclad run records, 0 new completion markers, and 0 victories. The incomplete batch was deliberately not continued to 25.
...
- Decision trace: 4,547 cutoff-bounded rows. Screens include 26 `HAND_SELECT` and 98 `GRID` rows; the trace contains no row whose `confirm` command is absent from its captured available-command list, which is consistent with the rejection occurring between decision capture and queued execution.
...
- GRID: 97 selector returns, all exactly `1/1 remaining`; zero selector cardinality mismatches. The separate 98th GRID trace row is the `ProceedAction` that became the rejected queued `confirm` after the screen transition.
