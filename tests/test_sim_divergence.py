...
def test_writhing_mass_attack_damage_ignores_random_intent_change(
    monkeypatch, tmp_path
):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
...
def test_player_intangible_caps_hemokinesis_hp_loss(monkeypatch, tmp_path):
    ...
    intangible = Power("IntangiblePlayer", "Intangible", 1)
    before = _game(
        floor=22,
        turn=2,
        player=SimpleNamespace(
        ...
        monsters=[_monster(name="Byrd", monster_id="Byrd", hp=40, damage=0)],
    )
    actual = _game(
        floor=22,
        turn=2,
        ...
        hand=[],
        monsters=[_monster(name="Byrd", monster_id="Byrd", hp=20, damage=0)],
    )
...
def test_x_cost_skill_consumes_all_current_energy(monkeypatch, tmp_path):
    ...
    before = _game(
        floor=12,
        turn=1,
        player=SimpleNamespace(current_hp=78, max_hp=80, block=0, energy=3),
        hand=[transmutation],
        monsters=[
            _monster(name="Sneaky Gremlin", monster_id="GremlinThief", hp=14, damage=9),
            _monster(
                name="Fat Gremlin",
...
def test_headbutt_rage_block_settles_after_card_select(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    before = _game(
        ...
        player=SimpleNamespace(
            ...
            block=0,
            energy=5,
            powers=[Power("Rage", "Rage", 5)],
        ),
        hand=[headbutt, defend],
        monsters=[_monster(name="Slime Boss", monster_id="SlimeBoss", hp=134)],
    )
    select_screen = _game(
        floor=16,
        turn=1,
        ...
        hand=[defend],
        monsters=[_monster(name="Slime Boss", monster_id="SlimeBoss", hp=125)],
    )
    after_select = _game(
        floor=16,
        turn=1,
...
def test_body_slam_uses_current_block_as_damage(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    before = _game(
        floor=7,
        turn=1,
        player=SimpleNamespace(current_hp=43, max_hp=80, block=10, energy=1),
        hand=[body_slam],
        monsters=[
            _monster(
                name="Slaver",
    ...
    actual = _game(
        floor=7,
        turn=1,
        player=SimpleNamespace(current_hp=43, max_hp=80, block=10, energy=0),
        hand=[],
        monsters=[
            _monster(
                name="Slaver",
...
def test_body_slam_zero_block_still_uses_strength(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
...
def test_havoc_uses_known_draw_pile_top_second_wind_block(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    before = _game(
        ...
        hand=[defend_one, havoc, strike, defend_two],
        draw_pile=[strike, top_second_wind],
        monsters=[_monster(name="Looter", monster_id="Looter", hp=43, damage=10)],
    )
    actual = _game(
        floor=18,
        turn=2,
        player=SimpleNamespace(current_hp=80, max_hp=80, block=10, energy=1),
        hand=[strike],
        draw_pile=[strike],
        monsters=[_monster(name="Looter", monster_id="Looter", hp=43, damage=10)],
    )
...
def test_havoc_top_reaper_healing_is_scaled_by_magic_flower(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    top_reaper = _card(name="Reaper", card_id="Reaper", cost=2, damage=4)
    before = _game(
        floor=18,
        turn=2,
        player=SimpleNamespace(current_hp=80, max_hp=90, block=11, energy=0),
        hand=[havoc],
        draw_pile=[bottom_defend, top_reaper],
        monsters=[_monster(name="Chosen", monster_id="Chosen", hp=72, damage=0)],
        relics=[_relic("Magic Flower")],
    )
    actual = _game(
        floor=18,
        turn=2,
        player=SimpleNamespace(current_hp=86, max_hp=90, block=11, energy=0),
        hand=[],
        draw_pile=[bottom_defend],
        monsters=[_monster(name="Chosen", monster_id="Chosen", hp=68, damage=0)],
        relics=[_relic("Magic Flower")],
    )
...
def test_havoc_top_attack_random_target_multi_monster_is_not_divergence(
    ...
    before = _game(
        floor=24,
        turn=5,
        player=SimpleNamespace(current_hp=34, max_hp=80, block=5, energy=0),
        hand=[havoc],
        draw_pile=[top_strike],
        monsters=[
            _monster(name="Centurion", monster_id="Centurion", hp=40, damage=6),
            _monster(
                name="Mystic",
                monster_id="Healer",
                hp=31,
...
def test_havoc_exhausted_top_card_triggers_feel_no_pain_block(monkeypatch, tmp_path):
    ...
    feel_no_pain = Power("Feel No Pain", "Feel No Pain", 3)
    before = _game(
        floor=27,
        turn=1,
        player=SimpleNamespace(
        ...
        monsters=[_monster(name="Snake Plant", monster_id="SnakePlant", hp=79, damage=8)],
    )
    actual = _game(
        floor=27,
        turn=1,
...
def test_havoc_top_clash_with_non_attack_hand_does_not_expect_damage(
    monkeypatch,
    tmp_path,
):
    trace_path = tmp_path / "sim_divergence.jsonl"
    ...
    before = _game(
        floor=16,
        turn=4,
        player=SimpleNamespace(current_hp=54, max_hp=80, block=11, energy=1),
        hand=[defend, havoc],
        draw_pile=[top_clash],
        monsters=[_monster(name="Hexaghost", monster_id="Hexaghost", hp=188, damage=6)],
    )
    actual = _game(
        floor=16,
        turn=4,
        player=SimpleNamespace(current_hp=54, max_hp=80, block=11, energy=0),
        hand=[defend],
        draw_pile=[],
        monsters=[_monster(name="Hexaghost", monster_id="Hexaghost", hp=188, damage=6)],
    )
...
def test_reaper_healing_is_scaled_by_magic_flower(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()

    reaper = _card(name="Reaper", card_id="Reaper", cost=2, damage=4)
    before = _game(
        floor=18,
        turn=2,
        player=SimpleNamespace(current_hp=80, max_hp=90, block=11, energy=2),
        hand=[reaper],
        monsters=[_monster(name="Chosen", monster_id="Chosen", hp=72, damage=0)],
        relics=[_relic("Magic Flower")],
    )
...
def test_hemokinesis_lethal_self_damage_prevents_attack_damage(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    before = _game(
        floor=31,
        turn=2,
        player=SimpleNamespace(current_hp=2, max_hp=80, block=5, energy=3),
        hand=[hemokinesis_plus],
        monsters=[
            _monster(
                name="Snake Plant",
    ...
    actual = _game(
        floor=31,
        turn=2,
        player=SimpleNamespace(current_hp=0, max_hp=80, block=0, energy=2),
        hand=[],
        monsters=[
            _monster(
                name="Snake Plant",
...
def test_end_turn_bronze_automaton_random_orb_summon_ignores_structure_boundary(
    monkeypatch, tmp_path
):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
...
def test_end_turn_bronze_automaton_random_orb_summon_reports_boss_hp_diff(
    monkeypatch, tmp_path
):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    ...
    assert any(key.endswith(".hp") for key in records[0]["diffs"])
...
def test_end_turn_regret_loses_hand_size_hp_through_block(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    before = _game(
        floor=33,
        turn=7,
        player=SimpleNamespace(current_hp=10, max_hp=80, block=19, energy=1),
        hand=[regret, burn],
        monsters=[
            _monster(
                name="The Champ",
    ...
    actual = _game(
        floor=33,
        turn=8,
        player=SimpleNamespace(current_hp=8, max_hp=80, block=0, energy=4),
        hand=[],
        monsters=[
            _monster(
                name="The Champ",
...
def test_end_turn_stone_calendar_counter_seven_damages_monsters(
    monkeypatch,
    tmp_path,
):
    trace_path = tmp_path / "sim_divergence.jsonl"
    ...
    before = _game(
        floor=16,
        turn=8,
        player=SimpleNamespace(current_hp=13, max_hp=80, block=7, energy=0),
        hand=[],
    ...
    actual = _game(
        floor=16,
        turn=9,
        player=SimpleNamespace(current_hp=11, max_hp=80, block=0, energy=3),
        hand=[],
...
def test_end_turn_mercury_hourglass_hits_guardian_defend_block(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    before = _game(
        ...
        monsters=[
            _monster(
                ...
                intent=Intent.DEFEND,
                move_id=6,
                powers=[Power("Mode Shift", "Mode Shift", 13)],
            )
        ],
    )
    actual = _game(
...
def test_end_turn_mercury_hourglass_does_not_trigger_after_player_death(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    before = _game(
        floor=16,
        turn=7,
        player=SimpleNamespace(current_hp=3, max_hp=80, block=0, energy=0),
        hand=[],
        relics=[_relic("Mercury Hourglass")],
        monsters=[
            _monster(
                ...
                intent=Intent.ATTACK,
                move_id=2,
                powers=[Power("Mode Shift", "Mode Shift", 19)],
            )
        ],
    )
    actual = _game(
        floor=16,
        turn=8,
        player=SimpleNamespace(current_hp=0, max_hp=80, block=0, energy=1),
        hand=[],
        relics=[_relic("Mercury Hourglass")],
        monsters=[
            _monster(
...
def test_end_turn_flame_barrier_reflects_zero_damage_multi_hit_attack(
    monkeypatch,
    tmp_path,
):
    trace_path = tmp_path / "sim_divergence.jsonl"
...
def test_charons_ashes_corruption_skill_exhaust_deals_aoe_damage(monkeypatch, tmp_path):
    trace_path = tmp_path / "sim_divergence.jsonl"
    monkeypatch.setenv("STS_SIM_DIVERGENCE_TRACE_FILE", str(trace_path))
    reset_pending_divergence()
    ...
    before = _game(
        ...
        player=SimpleNamespace(
            ...
            block=0,
            energy=3,
            powers=[Power("Corruption", "Corruption", -1)],
        ),
        hand=[defend],
        relics=[_relic("Charon's Ashes")],
        monsters=[
            _monster(name="The Champ", monster_id="Champ", hp=20, block=5),
            _monster(name="Cultist", monster_id="Cultist", hp=12, block=0),
        ],
    )
    actual = _game(
        floor=33,
        ...
        monsters=[
            _monster(name="The Champ", monster_id="Champ", hp=20, block=2),
            _monster(name="Cultist", monster_id="Cultist", hp=9, block=0),
        ],
    )
