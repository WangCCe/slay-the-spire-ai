...
class Player(Character):
    ...
    def from_json(cls, json_object):
        player = cls(
            coerce_int(json_object["max_hp"], 0),
            coerce_int(json_object["current_hp"], 0),
            coerce_int(json_object["block"], 0),
            coerce_int(json_object["energy"], 0),
        )
...
class Monster(Character):
    ...
    def from_json(cls, json_object):
        ...
        max_hp = coerce_int(json_object["max_hp"], 0)
        current_hp = coerce_int(json_object["current_hp"], 0)
        block = coerce_int(json_object["block"], 0)
        intent = Intent[json_object["intent"]]
        half_dead = json_object["half_dead"]
        is_gone = json_object["is_gone"]
        move_id = coerce_int(json_object.get("move_id", -1), -1)
        last_move_id = (
            coerce_int(json_object.get("last_move_id"), 0)
            if json_object.get("last_move_id") is not None
            else None
        )
        second_last_move_id = (
            coerce_int(json_object.get("second_last_move_id"), 0)
            if json_object.get("second_last_move_id") is not None
            else None
        )
        move_base_damage = coerce_int(json_object.get("move_base_damage", 0), 0)
        move_adjusted_damage = coerce_int(json_object.get("move_adjusted_damage", 0), 0)
        move_hits = coerce_int(json_object.get("move_hits", 0), 0)
