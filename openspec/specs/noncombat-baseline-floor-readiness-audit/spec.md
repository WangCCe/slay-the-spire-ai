...
- **WHEN** the immutable integrated study stops before a complete canary on an
  exact deterministic compatibility defect, a later source-complete audit and
  red/green regression close that defect and all other known static surfaces,
  Current and its weak control remain unchanged, every gate remains unchanged
  or stricter, partial rows remain non-authoritative, and every consumed seed
  is excluded
- **THEN** the verdict MAY be `ready_for_final_baseline_replication_proposal`
- **AND** it SHALL permit consideration of exactly one distinct versioned
  final replication without reopening or replacing the consumed identity

#### Scenario: Final replication contract is complete but unexecuted
- **WHEN** the unique successor binds the complete predecessor and repair
  lineage, deterministic fresh-cohort algorithm, candidate role, conservative
  support handling, unchanged-or-stricter gates, replay, bootstrap, stop,
  limits, publication, terminal lane rule, and separate execution approval
- **THEN** the verdict MAY be
  `ready_for_final_baseline_replication_preregistration`
- **AND** native loading, environment construction, seed access, execution,
  and formal RL SHALL remain unauthorized
...
- **WHEN** every non-teacher candidate is invalid, structurally incompatible,
  lacks a conservative evaluable episode contract, or the unique final Current
  replication has reached a non-passing or invalid terminal verdict
...
The audit SHALL treat a registered integrated study or final replication as
structural and floor evidence only when its immutable canary and holdout
contracts were followed, SHALL NOT infer success from partial or supported-only
rows, and SHALL distinguish the single approved pre-quality repair exception
from any complete or final result.
...
- **WHEN** the consumed result is `study_blocked` on
  `card_metadata_cost_invalid: Injury`, its canary is incomplete, its holdout
  is untouched, and the separately registered post-final-repair evidence
  satisfies every final-replication prerequisite
- **THEN** the audit MAY recommend exactly one distinct final replication
  proposal
- **AND** it SHALL NOT complete, retry, resume, replace, or reinterpret the
  consumed study or its partial rows

#### Scenario: Integrated study has any other negative result
- **WHEN** an integrated study stops at canary, is blocked for another reason,
  is interrupted, is invalid, or returns
...
- **AND** it SHALL NOT recommend a diagnostic, replacement seeds, threshold
  changes, or a same-question retry

#### Scenario: Final replication demonstrates a floor
- **WHEN** the canonical successor result is
  `replication_valid_with_baseline_floor`, every selected row is retained,
  replay and bootstrap identities match, and every absolute, paired, support,
  coverage, lifecycle, and publication gate passes
- **THEN** a later read-only refresh MAY mark the Current baseline-floor
  contract demonstrated
- **AND** target-supported outcomes and formal training SHALL remain
  independently blocked

#### Scenario: Final replication has any non-passing or invalid result
- **WHEN** the successor stops at canary, is blocked, interrupted, invalid, or
  returns `replication_valid_without_baseline_floor`
- **THEN** the audit SHALL keep the baseline policy domain blocked and classify
  Current as no longer eligible for another baseline attempt
- **AND** it SHALL NOT recommend repair, retry, replacement seeds, threshold
  changes, another comparator run, or a same-question successor
