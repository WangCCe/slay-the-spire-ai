# noncombat-route-card-residual-ranker Specification

## Purpose

Define the frozen legacy base, route/card-only bounded residual, exact
event/shop delegation, seed-grouped terminal gate, canonical per-fold
reporting, and no-authority verdicts for the terminal train-only residual POC.
