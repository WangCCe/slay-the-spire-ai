...
The system SHALL optionally append a deterministic registered encounter-identity feature
...
- **THEN** the report records the encoding, hash algorithm when applicable, bucket count, observed encounter-to-bucket assignments, canonical vocabulary hash when applicable, original and migrated parent hashes, and equivalence metrics
...
simulator parent through either the default raw-greedy action or an explicitly
selected frozen-parent deployment-guard action while preserving one-step TD as
the default.
