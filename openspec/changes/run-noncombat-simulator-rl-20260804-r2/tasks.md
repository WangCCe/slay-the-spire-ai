## 1. Publish The One-Shot Authorization
...
- [ ] 3.1 Revalidate the pushed authorization/preflight, exact module and DLL paths, free lease, output absence, and no relevant game/CommunicationMod process immediately before launch.
- [ ] 3.2 Invoke the exact registered execute command once, allowing native-first pre-start validation and then at most `28,800` cumulative CPU seconds and `5,504` registered episodes.
- [ ] 3.3 Wait for terminal completion; if externally interrupted, resume only the same logical execution from canonical state without changing controls or cumulative bounds.
- [ ] 3.4 Run the standalone verifier in a fresh process and require the exact reached terminal artifact inventory, journal, resource accounting, cohort access, and verdict.

## 4. Close Out The Evidence

- [ ] 4.1 Review terminal metrics, model, evaluation if reached, report, manifest, unsupported accounting, and learning-signal result without tuning or reinterpretation.
- [ ] 4.2 Prove r1, r2 registration/authorization, CommunicationMod config, and production checkpoints remain byte-unchanged and no gameplay run was launched.
- [ ] 4.3 Update project direction, sync the accepted requirement, archive this change, and run strict full-tree OpenSpec plus `git diff --check`; do not run pytest or gameplay because source is unchanged.
