...
The latest saved dataset contains 373 decisions and 216 matched outcome rows, but those matched rows come from only 6 unique runs and contain no victories. Shop evidence spans only 3 matched runs. Sample count therefore overstates independent evidence. In addition, the current behavior policy is largely deterministic and the sample schema does not consistently record policy identity or a known action probability, so reliable off-policy evaluation is not yet possible.
...
2. Re-export samples from the frozen baseline's clean batches into a new, separately named v2 dataset.
