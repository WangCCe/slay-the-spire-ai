...
The 2026-07-15 v1 outcome-evidence study is permanently blocked and cannot be resumed, while the underlying evidence question remains unresolved. Three protocol-era pre-lock v2 qualification identities are immutable failures: r1 stopped on an external Windows log-sharing error, r2 proved that the registered 30-second readiness deadline was too short for cold CommunicationMod startup, and r3 reached a valid ready record but no release was preserved. R4 is obsolete; r5 and r6 are retired static-only launcher-timeout identities; r7 is retired after `source_validation_failed`; and r8 is retired after reaching `source_verified` and then failing `request_validation_failed` before active-request publication. The r7 source defect was fixed separately, but the r8 request-validation root cause is not established. Every identity remains non-authorizing, the registered study root is absent, and collection cannot start.
...
- Preserve r4 through r8 as immutable pre-lock history. R8 source snapshot S `519cf55fa17706e7faee05a05c3a9c85f4238b75`, direct-child R `1d805b450862d2b5f34aeb2d4bbbeb9884510c84`, six-file root, standalone verifier, and independent closeout prove only a consumed `request_validation_failed` boundary with exact restoration. Diagnose that boundary in a separate regression-backed change before any new qualification amendment is proposed; do not retry r8 or prepare r9 here.
...
- Primary code and tools: historical snapshots and request bindings through r8 remain immutable. The r8 closeout establishes no request-validation root cause and records no active replacement candidate.
- Planning and evidence artifacts: one v2 study identity, historical registration reviews, immutable r1-r8 qualification evidence, one still-absent registered study root, and no current start, collection, OPE, training, gameplay, or promotion authority.
...
- Rollback boundary: before `start`, preserve r1-r8 and every superseded registration, request, configuration, verifier, and closeout byte, and collect no evidence. R8 cannot be retried, relabeled, repaired in place, or used as authority for r9. A future replacement requires a separate diagnosis/fix and explicit qualification amendment. Once `start` publishes any registered study artifact, any integrity failure follows the registered blocked or recovery path and restores live configuration only after independent verification.
