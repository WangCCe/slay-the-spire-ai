...
The 2026-07-15 v1 outcome-evidence study is permanently blocked and cannot be resumed, while the underlying evidence question remains unresolved. The recovery change now provides a launchable v2 handshake and an independent blocked-closeout verifier, so the next step is a separately reviewed v2 registration with a new artifact root rather than training, policy tuning, or reuse of v1 evidence.
...
- Add a pre-collection qualification gate that replays the canonical registration, dry-runs every registered child command, and proves the real CommunicationMod child can complete the no-action attempt/ready/release handshake without claiming a ledger slot or creating gameplay evidence.
...
- Planning and evidence artifacts: one committed v2 registration and registration review, one external pre-lock qualification record that is copied into the repository only after the run-lock window closes, one new external study root, blinded monitor artifacts, and a deterministic final closeout and verifier audit.
...
- Rollback boundary: before `start`, reject or abandon the v2 registration, restore the pre-study CommunicationMod baseline, and collect no evidence. Once `start` publishes any registered study artifact, any integrity failure follows the registered blocked or recovery path and restores live configuration only after independent verification.
