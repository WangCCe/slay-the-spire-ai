...
3. Complete a separate offline-first change that reproduces the exact r8 `request_validation_failed` path and proves or rejects an implementation defect with a regression. Do not retry r8 or prepare r9 as diagnosis.
4. Only after a proven defect is minimally fixed and independently reviewed may another explicit amendment prepare a previously absent replacement root and canonical request with regenerated source and implementation bindings, an exact CommunicationMod baseline, and a new direct-child review commit.
...
6. Preserve and replay the completed r7 diagnosis/fix and the retired r8 `request_validation_failed` closeout. Diagnose r8 in a separate regression-backed change; only a later explicit amendment may prepare and qualify a previously absent replacement identity. Replay all historical roots plus that successful terminal and independent attestation; require the exact qualified HEAD and all static/live bindings; only then create the run lock and ledger without an intervening commit.
...
The study identity, schedule, rates, thresholds, target, estimator, r1-r8 qualification history, 120/10 handshake bounds, stop behavior, and authority limits remain fixed. No active replacement, successful terminal, study authority, or live authority exists. The next open question is the exact invariant behind r8 `request_validation_failed`; diagnosis and any resulting fix belong to a separate regression-backed change before another qualification amendment is considered.
