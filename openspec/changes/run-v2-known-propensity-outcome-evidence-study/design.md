...
- Retired r8 qualification root: `D:\SteamLibrary\steamapps\common\SlayTheSpire\noncombat_outcome_evidence_expansion_20260716_v2_qualification_r8` (claim plus three valid stages, then `request_validation_failed`; no active request or terminal)
- R8 source snapshot S: `519cf55fa17706e7faee05a05c3a9c85f4238b75`
- R8 direct-child review commit R: `1d805b450862d2b5f34aeb2d4bbbeb9884510c84`
- R8 repository closeout: `reports/noncombat_outcome_evidence_expansion_20260716_v2_r8_qualification_closeout.json`
- Retired r7 qualification root: `D:\SteamLibrary\steamapps\common\SlayTheSpire\noncombat_outcome_evidence_expansion_20260716_v2_qualification_r7` (`source_validation_failed`, separately diagnosed and fixed; no active request or terminal)
...
The previously absent r6 root still contains only its 949-byte canonical static config. Its request v2 binds source snapshot S `2936c547bd7917fdbbc487470326716129e3fbe2`, the unchanged registration and implementation map, and a deliberate 535-byte CommunicationMod baseline that differs from the original only by explicit `maxInitializationTimeout=120`. Direct-child R `9542af3aae8f93fa29eeaeefabcc5a1860a0107a` passed 95 source-only verifier checks and was pushed before one real launch. CommunicationMod parsed the reviewed 20-token command and explicit timeout, waited 120 seconds without receiving `ready`, and killed the silent external process before the active request was published. R6 produced no active request, attempt, ready, release, terminal, marker, run, checkpoint, global-log, study-root, run-lock, or ledger change. After Java stopped, the exact 535-byte baseline and zero-drift request-bound isolation were restored. R6 is retired without retry.

R7 then proved an internal `source_validation_failed` boundary after `runner_entered`. The separate `fix-qualification-source-byte-normalization` change reproduced the mixed-line-ending and built-in `binary` failures, implemented raw-first validation, passed the registered gate, and was archived without retrying r7. R8 reused that fix under S/R above and reached `source_verified`, but then failed `request_validation_failed` before active-request publication. Its immutable chain proves at least one live boundary while the same-session operator journal, not independent evidence, records one invocation. The six-file root, standalone verifier, independent attestation, and imported closeout agree on `retired_after_live_boundary`; exact CommunicationMod and protected inventories were restored, target processes are zero, and the registered study root remains absent. The separate request-source launch-binding change proves and fixes the implementation cause without changing r8. R8 cannot be retried or used to prepare r9, and there is no active replacement candidate.
...
There is no active prepared replacement candidate. The separate r8 diagnosis proved that its CLI bound `--request` to the absent active publication path instead of the reviewed repository source; the regression-backed source fix now provides one canonical builder and exact validator for that binding. The later feasibility audit binds the current v2 registration and frozen B3-B7 readiness artifact, observes 125 complete trajectories, one raw victory, and zero target-supported victories, and reports `not_demonstrated`. It keeps replacement qualification and study launch blocked before another operational identity is considered. Any later run lock, ledger, configs, traces, manifests, pool, estimates, and closeout may reference only a future candidate reviewed after both the source fix and a current source-comparable feasibility pass. The three protocol-era failed qualification roots, obsolete r4 root, and retired r5-r8 roots are pre-lock operational history and can never enter the v2 pool or satisfy a launch gate. The immutable v1 study root may be verified read-only as a historical compatibility check, but no v1 trajectory or decision may enter the v2 pool.
...
1. Preserve and replay r1-r8 from their immutable roots and external closeouts. Require the registered study root to remain absent.
2. Preserve the completed isolation, observability, and r7 source-byte repairs plus every historical source/request binding; none authorizes reuse of a retired identity.
3. Preserve the completed offline-first r8 diagnosis and source fix: the canonical qualifier suffix must bind `--request` to `request_source_path`, reject `request_path`, and pass exact semantic-vector validation. Do not retry r8 or reinterpret its retired boundary.
4. Only after that fix is archived and independently reviewed may another explicit amendment prepare a previously absent replacement root and canonical request with regenerated source and implementation bindings, an exact CommunicationMod baseline, a semantically validated final command vector, and a new direct-child review commit.
...
All qualification attempts are operational evidence, not registered slots or outcome samples. R1-r8 are immutable failed, obsolete, or retired identities and none can be retried, reused, or interpreted as a successful qualification. None weakens the real per-slot preclaim handshake, which remains the authority for slot launch.
...
6. Preserve and replay the completed r7 diagnosis/fix, the retired r8 `request_validation_failed` closeout, the separate canonical request-source launch-binding fix, and the bound feasibility `not_demonstrated` report. Do not prepare a replacement identity while feasibility remains undemonstrated. Only a later explicit amendment following a current source-comparable feasibility pass may prepare and qualify a previously absent replacement identity, and its exact final vector must pass the semantic gate before publication. Replay all historical roots plus that successful terminal and independent attestation; require the exact qualified HEAD and all static/live bindings; only then create the run lock and ledger without an intervening commit.
...
The study identity, schedule, rates, thresholds, target, estimator, r1-r8 qualification history, 120/10 handshake bounds, stop behavior, and authority limits remain fixed. The r8 failing invariant is resolved by the separate canonical request-source launch-binding fix. The current feasibility result is `not_demonstrated`, and no active replacement, successful terminal, study authority, or live authority exists. The next open question is whether to improve and re-baseline the policy, revise the outcome/reward evidence contract, or register a materially different study before any replacement qualification is reconsidered.
