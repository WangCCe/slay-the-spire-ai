...
R7 then proved an internal `source_validation_failed` boundary after `runner_entered`. The separate `fix-qualification-source-byte-normalization` change reproduced the mixed-line-ending and built-in `binary` failures, implemented raw-first validation, passed the registered gate, and was archived without retrying r7. R8 reused that fix under S/R above and reached `source_verified`, but then failed `request_validation_failed` before active-request publication. Its immutable chain proves at least one live boundary while the same-session operator journal, not independent evidence, records one invocation. The six-file root, standalone verifier, independent attestation, and imported closeout agree on `retired_after_live_boundary`; exact CommunicationMod and protected inventories were restored, target processes are zero, and the registered study root remains absent. R8 cannot be retried or used to prepare r9, and there is no active replacement candidate.
...
There is no active prepared replacement candidate. The separate r8 diagnosis proved that its CLI bound `--request` to the absent active publication path instead of the reviewed repository source; the regression-backed source fix now provides one canonical builder and exact validator for that binding. Any later run lock, ledger, configs, traces, manifests, pool, estimates, and closeout may reference only a future candidate reviewed after that fix. The three protocol-era failed qualification roots, obsolete r4 root, and retired r5-r8 roots are pre-lock operational history and can never enter the v2 pool or satisfy a launch gate. The immutable v1 study root may be verified read-only as a historical compatibility check, but no v1 trajectory or decision may enter the v2 pool.
...
6. Preserve and replay the completed r7 diagnosis/fix, the retired r8 `request_validation_failed` closeout, and the separate canonical request-source launch-binding fix. Only a later explicit amendment may prepare and qualify a previously absent replacement identity, and its exact final vector must pass the new semantic gate before publication. Replay all historical roots plus that successful terminal and independent attestation; require the exact qualified HEAD and all static/live bindings; only then create the run lock and ledger without an intervening commit.
...
The study identity, schedule, rates, thresholds, target, estimator, r1-r8 qualification history, 120/10 handshake bounds, stop behavior, and authority limits remain fixed. The r8 failing invariant is resolved by the separate canonical request-source launch-binding fix. No active replacement, successful terminal, study authority, or live authority exists. The next open question is whether to approve a later explicit amendment for one previously absent replacement identity under the corrected semantic-vector gate.
