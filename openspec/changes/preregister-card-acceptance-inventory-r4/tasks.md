...
- [x] 2.4 Commit and push dispatch/source/preflight/request artifacts and reviews as one immutable request boundary; do not edit source or tests.
...
- [x] 3.3 Commit and push delegated approval, authorization, and reviews together; verify current HEAD equals pushed `origin/master` and remains tracked-clean.
...
- [x] 4.4 After the gate push, perform a separate no-write recheck of HEAD/origin, tracked cleanliness, gate bytes, process absence, terminal-r3 preservation, and every r4 absence condition. Stop before launch if any code/test edit or ambiguous content access is required.

## 5. Materialize, Verify, Or Close Terminal

- [x] 5.1 Invoke the exact r4 `build-inventory` command under the 3,600-second outer observer. Apply only the established bounded ordinal-two pre-start reinvocation rule after an independently proven external-only pre-receipt failure; never retry automatically. Any ambiguity, second pre-start failure, existing/partial receipt, post-receipt failure, compactness failure, or completion failure closes r4 without retry, resume, tuning, raised bounds, replacement, verification, or registration.
- [x] 5.2 On build success only, publish a distinct verification request/review and authorization/review, capture a fresh verification launch observation, then run `verify-inventory` once to rescan exact registered Git bytes and independently reconstruct compact source counts, exclusions, fixed cohorts, role/whole digests, authority, receipt, output closure, and byte ceilings without selection or r3 artifact-content access. Terminal N/A: commit review executed an unregistered verifier command before authorization/launch publication, so no verification was accepted and no reinvocation is permitted.
- [x] 5.3 Run the standalone successor verifier over the verified compact artifact. Only exact source-only plus structural agreement may publish and independently review one canonical all-false r4 registration, complete parent task 6.2, leave 6.3 unchecked, and commit/push the bounded success outcome. Terminal N/A: r4 has no accepted source-only verification and cannot publish a registration.
- [x] 5.4 On any terminal failure, publish and independently review only bounded failure evidence; preserve all predecessor/current evidence, leave parent 6.2 unchecked, and grant no training or downstream authority.
