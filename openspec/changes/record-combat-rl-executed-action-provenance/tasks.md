...
- [x] 3.4 Register and collect one bounded fresh zero-update production-r16 replay cohort, retaining `.run`, decision trace, guard logs, and checkpoint evidence without training.
- [x] 3.5 Publish a report reconciling nonzero legal override rows with direct, replacement, and takeover evidence, then make a separate training go/no-go decision.
