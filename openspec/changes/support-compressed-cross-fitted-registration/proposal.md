...
- Add a backward-compatible compact registration schema that replaces the embedded historical inventory with exact bindings to one independently verified readiness publication commit, verification receipt, report, and deterministic `gzip-mtime-zero-v1` candidate artifact while retaining the complete 512-seed schedule.
- Require source-only preflight to read all three readiness artifacts from their exact immutable Git commit and paths; cross-bind the complete registration source inventory to that readiness source; enforce the existing 64 MiB stored and 512 MiB candidate-canonical ceilings; and reject noncanonical JSON, nondeterministic gzip, trailing or oversized data, non-`go` readiness, authority drift, source drift, schedule drift, or cohort collisions before dependency loading.
