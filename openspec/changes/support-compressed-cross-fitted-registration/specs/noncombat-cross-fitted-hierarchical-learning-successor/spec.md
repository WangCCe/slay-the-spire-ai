...
Any new empirical-successor registration SHALL use a compact canonical registration schema that retains the complete 512-seed schedule and replaces an embedded historical inventory with exact bindings to one immutable pushed readiness publication commit, report, and deterministic-gzip candidate artifact. The binding SHALL include the publication commit, canonical repository paths, stored sizes and SHA-256 digests, candidate canonical size and SHA-256, encoding, and readiness identity. The publication commit SHALL contain both exact artifacts, descend from the readiness source commit, and be an ancestor of the current pushed head. Historical embedded-inventory registrations SHALL remain verifiable against their own registered Git source but SHALL NOT be used to publish a new successor identity.
...
- **THEN** the source-only builder may emit one all-false compact registration whose canonical digest transitively binds both readiness artifacts without embedding the historical inventory
...
For a compact registration, source-only inspection, request rendering, and execution preflight SHALL read the readiness report and candidate artifact from their exact publication-commit Git paths. They SHALL require the registered source commit, exact all-false authority, `go` decision, proposal eligibility, readiness identity, candidate binding, complete historical inventory, canonical fresh schedule, consumed cohort, zero collisions, and registration schedule to agree before Torch, native, model, environment, fitting, training, or seed access is possible. Every registration source-inventory row SHALL match its blob under the readiness source commit, and readiness's control-plane, terminal-verifier, seed-helper, and successor-contract bindings SHALL match the exact registered implementation and contract identities.

#### Scenario: Exact compact evidence passes
- **WHEN** the pushed report and deterministic-gzip candidate match every registered path, byte binding, source identity, authority, freshness, disjointness, and schedule term
...
- **WHEN** a publication commit, ancestry relation, report or candidate path, byte, digest, size, encoding, source commit, source row, required readiness binding, authority bit, decision, eligibility flag, readiness identity, seed, chunk, inventory digest, consumed-cohort term, or collision result differs
...
- **WHEN** the verifier cannot read the exact pushed report or candidate or independently reproduce its registered identity
