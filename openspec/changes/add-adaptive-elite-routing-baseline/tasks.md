...
- [ ] 1.5 Run exactly one clean-source requalification with the production Windows interpreter across seven cases, exact `10` warm-up / `100` measured-pair-per-case protocol (700 measured pairs total), and unchanged `25 ms` median / `100 ms` maximum limits. The benchmark writes `reports/adaptive_route_candidate_poc_20260721_attempt-2_clean.json`; generate `reports/adaptive_route_candidate_poc_20260721_attempt-2_clean.md` from that exact result without overwriting canonical or `attempt-1-fail` evidence. Proceed only if it passes, otherwise stop this change without another retry.
