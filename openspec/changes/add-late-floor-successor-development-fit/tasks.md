...
- [ ] 1.1 Add failing fixtures for exact source, native, parent, predecessor,
  target, contamination, cohort, output, and authority registration binding.
- [ ] 1.2 Add failing fixtures for fixed battle-index-10 partition collection,
  lineage seed isolation, deterministic merge, and row-reference preservation.
- [ ] 1.3 Add failing fixtures for context-only fresh projection round trips,
...
- [ ] 2.1 Implement source-only registration and preflight for fit seeds
  `288000..290047`, calibration seeds `290048..290559`, and fresh seeds
  `291000..292023` at battle index 10.
- [ ] 2.2 Reuse the existing first-successor collector for the three fixed
...
- [ ] 2.3 Deterministically append new fit and calibration rows to their bound
...
- [ ] 3.1 Publish a canonical fresh context projection whose identity is bound
  to the sealed fresh corpus without exposing policy labels or metrics.
- [ ] 3.2 Evaluate every unchanged context-support and integrity gate against
...
- [ ] 3.3 Implement an exact fit registration that stops before optimizer
...
- [ ] 3.4 Enforce fresh corpus deserialization only after both arms and
...
- [ ] 4.1 Run focused collector, merge, projection, support, fit-wrapper, and
...
- [ ] 4.3 Run one timed commit gate for the cohesive source boundary, record its
