## 1. Baseline And Correctness Repair

- [x] 1.1 Record the 2026-08-27 raw-full and commit-profile counts, durations, failure clusters, slow-test evidence, invalidated qualification, and unchanged authority in a repository report.
- [x] 1.2 Add a focused Reaper regression fixture repair that supplies the complete current healing state without changing simulator behavior, then run its narrow test node.
- [x] 1.3 Audit the current-baseline replication source binding against the current bound files; rebind only after proving the intended historical semantic surface is unchanged, then run the complete focused replication file.
- [x] 1.4 Audit reachable event-semantics Current-policy AST drift against the bound contract; update identities only after proving registered event behavior is unchanged, then run the reachable semantics and bridge-focused files.

## 2. Timing Evidence

- [x] 2.1 Add RED runner regressions for timing-report path validation, default argv compatibility, telemetry selection equivalence, deterministic per-file aggregation, nonzero pytest result preservation, and no retry.
- [x] 2.2 Implement opt-in run-scoped legacy JUnit capture and deterministic JSON timing publication in `scripts/run_test_gate.py` without changing default profile argv or return codes.
- [x] 2.3 Document timing-report usage and update the routine validation table so narrow coherent changes use focused coverage plus `commit`, while full remains an explicit complete boundary.
