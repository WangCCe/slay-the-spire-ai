...
- [x] 3.1 Add the frozen completion constants and strict receipt/output/completion validators using only standard-library operations, one fixed-buffer streaming inventory-byte hash, and no inventory parsing or reconstruction.
- [x] 3.2 Route only build/verify CLI success through the bounded completion builder; keep dispatch bytes, operation behavior, durable output, receipt ordering, and exceptions unchanged.
