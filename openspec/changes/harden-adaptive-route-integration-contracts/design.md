...
1. Resolve a usable active origin and validate the existing absolute route-history prefix. At an act's initial map choice, only an absent current node or an object with integer `y=-1`, together with advertised `y=0` next nodes, is a valid `start_y=0` origin with an empty current-act prefix. A stale complete `map_route` from the previous act is ignored rather than copied or treated as current-act history. Objects with missing/non-integer `y` or `y<-1`, and absent initial nodes with nonzero-row next nodes, are invalid before any builder call.
...
This permits recovery from an irrelevant malformed earlier/unreachable node that the legacy mid-act planner never visits, and it preserves first-map recovery without inventing a current node or reusing a previous-act prefix. After a valid adaptive initial route commits, map-choice action selection uses route index `0` for either an absent current node or the exact `y=-1` start sentinel. This special case is adaptive-only and does not normalize malformed legacy-mode current-node state. It does not recursively retry. Invalid current-act history, a missing/invalid origin, selected nodes whose internal coordinates differ from their map lookup keys, an exception from the conservative builder, invalid returned route data, or an unexpected selector/programming error still propagates without route/metadata/log mutation.
