...
- [ ] 1.1 Replace the irrelevant earlier-node propagation characterization with failing mid-act, floor-14, and first-map regressions proving one conservative fallback call, no repeated whole-map validation, preserved absolute or empty initial history, `start_y=0` first-map semantics, `candidate_generation_failed`, and no pre-commit mutation.
