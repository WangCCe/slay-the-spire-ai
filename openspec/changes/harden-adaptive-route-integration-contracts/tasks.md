...
- [ ] 1.1 Replace the irrelevant earlier-node propagation characterization with failing mid-act, floor-14, and first-map regressions proving one conservative fallback call, no repeated whole-map validation, preserved absolute mid-act history, an empty current-act `start_y=0` prefix, ignored previous-act `map_route`, `candidate_generation_failed`, and no pre-commit mutation.
- [ ] 1.2 Retain and extend regressions proving absent current nodes with nonzero-row next nodes, invalid current-act history, conservative-builder exceptions, invalid fallback output, and unexpected selector errors propagate without retry, route/metadata mutation, or adaptive decision log.
