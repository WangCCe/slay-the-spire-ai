...
- [ ] 1.2 Retain and extend regressions proving absent current nodes with nonzero-row next nodes, lookup-key/node-coordinate mismatches at the active origin and selected route, invalid current-act history, conservative-builder exceptions, invalid fallback output, and unexpected selector errors propagate without retry, route/metadata mutation, or adaptive decision log.
