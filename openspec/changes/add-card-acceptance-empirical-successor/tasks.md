...
- [x] 4.1 Implement the separately authorized `build-inventory` operation, explicit historical seed-source registry, output-root exclusions, canonical provenance scanner, fresh ascending `512/128/512` selector, and distinct post-build `verify-inventory` reconstruction that cannot select or materialize a cohort.
- [x] 4.2 Add regressions proving consumed, reserved, failed-access, and prior untouched holdout seeds remain excluded and generated candidate/staging/scratch roots cannot be recursively ingested.
- [x] 4.3 Implement the at-most-once 128-pair canary with write-once first-run arm output commitments, exact replay against those registered bytes, legality/control checks, both `64/2/0.95` concentration gates, and the exact unit-advantage family-only shadow Adam algorithm.
