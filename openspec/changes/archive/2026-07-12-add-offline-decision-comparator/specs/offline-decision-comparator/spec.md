...
The system SHALL provide an offline way to normalize operating-decision samples for shop, event, route, and card-reward decisions from local `.run` records, decision trace JSONL rows, or constructed fixtures.
...
The system SHALL generate a readable difference report that shows our current choice, Bottled-style reference choice, difference reason, confidence, and ranked follow-up issues, while preserving the no-repair-first Phase 1 boundary.
...
- **AND** the ranked section SHALL include 3-5 issues when supported by evidence, or explicitly state why fewer are justified

#### Scenario: Enforce no-repair gate
- **GIVEN** the report contains only partial-evidence, low-confidence, isolated, or combat-play differences
- **WHEN** the report is generated
- **THEN** it SHALL state that no gameplay-code fix is recommended yet
- **AND** it SHALL NOT modify `agent.py`, train models, tune parameters, or alter live gameplay configuration
