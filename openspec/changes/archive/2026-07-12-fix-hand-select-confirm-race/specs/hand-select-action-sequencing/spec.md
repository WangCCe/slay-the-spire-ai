...
### Requirement: GRID confirmation timing is unchanged
The action system SHALL preserve the existing non-blocking optional-confirm
behavior for GRID selections.

#### Scenario: GRID selection queues confirmation
- **WHEN** `CardSelectAction` handles a GRID selection
- **THEN** its optional confirmation SHALL retain the existing GRID readiness contract
- **AND** this HAND_SELECT fix SHALL NOT require a GRID confirmation to wait for game readiness
