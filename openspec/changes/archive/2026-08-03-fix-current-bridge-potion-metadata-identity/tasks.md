...
- [x] 1.2 Add red focused regressions for all three registered aliases, mapped
...
- [x] 1.3 Write a durable offline audit report that separates the observed r2
  `Elixir Potion` failure from the two additional static compatibility findings
  and records that the other 39 native names match metadata.

## 2. Narrow Compatibility Repair

- [x] 2.1 Add one closed stable-ID mapping containing the exact expected native
  and canonical metadata names for `ELIXIR_POTION`, `FAIRY_POTION`, and
  `GAMBLERS_BREW`.
- [x] 2.2 Change only the failed potion metadata lookup branch to require both
...
- [x] 2.3 Run the red regressions to green and review the implementation diff for
  broad normalization, native adapter changes, diagnostic changes, or unrelated
  policy behavior.

## 3. Verification And Closeout

- [x] 3.1 Run focused bridge and adjacent hydration pytest with a fresh writable
