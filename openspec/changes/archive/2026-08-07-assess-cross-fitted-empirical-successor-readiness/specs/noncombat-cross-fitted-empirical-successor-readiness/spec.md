...
conversion and rejecting a JSON string representation, apply an immutable multiplier of 3.0, and compute the projected total as
10,096.356 seconds with 4,303.644 seconds of margin. It SHALL NOT infer or tune
the budget from observed rehearsal timings.
...
started receipt exactly matches its own source, audit id, and paths. One pushed source identity SHALL produce at most one canonical
readiness attempt regardless of changed paths or audit id. A failure after the
claim and before completed installation SHALL write a typed all-false terminal receipt; independently verified
staging SHALL write a verified receipt before atomic installation. A typed
`no_go`, child timeout, corrupt inventory, or failed independent verification
SHALL NOT be rerun with changed paths, thresholds, inputs, cohort, or source
under the same identity. A verified `go` SHALL make only a separate empirical
registration proposal eligible for review.
