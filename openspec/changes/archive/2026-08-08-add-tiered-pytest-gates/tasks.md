...
- [x] 1.2 Add failing tests for profile command construction, current-interpreter use, unique profile-prefixed repository-local `basetemp`, ignored generated state, pre-executor command output, list and dry-run output, elapsed-time reporting, and pytest exit-code propagation.
...
- [x] 2.3 Implement serial pytest command construction and execution with `no:cacheprovider`, a unique profile-prefixed ignored `basetemp` per invocation, flushed pre-executor command reporting, duration reporting, and unchanged pytest exit status.
- [x] 2.4 Implement `--list` and `--dry-run` without launching pytest.

## 3. Qualification And Documentation

- [x] 3.1 Measure each candidate heavy file independently, keep `full_only` whole-file and minimal, and record a rationale for every exclusion.
