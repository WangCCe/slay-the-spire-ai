...
## 6. One Logical Execution And Closeout

- [x] 6.1 Run source-only preflight, then load native before Torch and execute
  at most one registered logical experiment on CPU; monitor process liveness
  only until exit and never read the active output root.
- [x] 6.2 After process exit, preserve and independently verify the exact
  positive, floor-only, negative, blocked, interrupted, resource, or invalid
  terminal evidence without retry, tuning, seed replacement, or repair.
- [x] 6.3 Publish a read-only interpretation and project-direction update that
  keeps formal readiness, Current baseline, target-supported outcomes,
  gameplay, OPE, qualification, loading, and promotion independent; sync specs,
  archive the change, run final applicable verification, and commit/push the
  canonical closeout.
