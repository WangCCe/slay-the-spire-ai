...
- [x] 4.2 Execute the bounded Windows CommunicationMod collection, preserve runtime evidence, restore the previous config, and stop project Python and game processes after completion or failure.
- [x] 4.3 Audit `.run`, debug, error, trace, inventory, checkpoint, action legality, proposal legality, class reconciliation, direct eval-parent parity, and SMDP span integrity; commit and push the collection decision.
...
- [x] 5.2 Commit and push the result, sync the delta specs, and archive the completed change. If a fixed gate fails, stop the corpus and record residual/separate-head as the next change.
