...
## Purpose
Extend the OptimizedAgent's beam search combat system to include potions as actionable options, enabling intelligent potion usage during difficult combat situations beyond just boss fights.
...
- **ai-combat/spec.md**:
  - Requirement: Survival-First Scoring (potion usage should improve survival)
  - Requirement: Threat-Based Targeting (damage potions use threat targeting)
  - Requirement: Two-Stage Action Expansion (potions included in FastScore)

## Notes

### Implementation Considerations
1. **Performance impact**: Adding ~5-10 potion actions at depth 0 increases beam width by <10%
2. **Potion effect data**: Communication Mod may not provide detailed effects; use lookup table
3. **State tracking**: Add `potions_used` field to SimulationState for scoring
4. **Testing priority**: Start with damage potions, then healing, then block

### Tuning Parameters
The following scoring thresholds should be tuned based on testing:
- Conservation penalty: 5-10 points (start with 5)
- Danger threshold for fallback: 0.6-0.7 (start with 0.6)
- Elite/boss bonus: 15-25 points (start with 20)

### Future Enhancements
- Consider deck synergies (e.g., value Entropy Potion higher in shiv decks)
- Account for potion scarcity (fewer potions remaining → higher conservation penalty)
- Special-case elite-specific potions (e.g., use Cultist Potion immediately vs Cultist)
